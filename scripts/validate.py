...
format_regex = re.compile(r'(?:%(?:\d+\$)?s|%(?:\d+\$)?d)') # Making non capturing groups so that findall returns actual result instead of tuple of gropus
...
def match(translated_format_identifier, english_format_identifier, name, lang, translated_text, original_text):
    ...
    if len(translated_format_identifier) != 0 or len(english_format_identifier) != 0:
        log(f'trans_list = {translated_format_identifier} and eng = {english_format_identifier} for name = {name} and lang = {lang}\n')
        for match in english_format_identifier:
            if match not in translated_format_identifier:
                return (name, lang, original_text, translated_text, english_format_identifier)

def contains_warning_char(name, lang, translated_text, original_text):
    if not translated_text:
        # Empty string should not come here
        raise ValueError(f'String is empty for name = {name} and text = {original_text}')
...
def check_xml_escaping(name, lang, translated_text, original_text):
    ...
    if escape(translated_text) != translated_text:
        return (name, lang, 'Wrong xml escaping', original_text, translated_text, f'Escaped string: {escape(translated_text)}')
    ...
    for i in range(len(input_tree_root)):
        ...
        if input_node.tag == 'string':
            ...
            if (not input_node.text.startswith('@string/')) and isTranslatable(input_node):
                ...
                log(f'Validating string with name = {name_attr}, prev string = "{previous_translated_text}" against en  string "{input_node.text}"')
                if previous_translated_text:
                    # Empty string throws exception
                    translated_matches = re.findall(format_regex, previous_translated_text)
                else:
                    translated_matches = []
                english_matches = re.findall(format_regex, input_node.text)
                ans.append(match(translated_matches, english_matches, name_attr, out_lang, previous_translated_text, input_node.text))
                ans.append(contains_warning_char(name_attr, out_lang, previous_translated_text, input_node.text))
                ans.append(check_xml_escaping(name_attr, out_lang, previous_translated_text, input_node.text))
            ...
            for j in range(len(input_node)):
                ...
                if(input_node[j].tag == 'item'):
                    if not input_node[j].text.startswith('@string/') and isTranslatable(input_node[j]):
                        ...
                        log(f'Validating {input_node.tag} with name = {name_attr}, prev string = "{previous_string}" against en  string "{input_node[j].text}"')

                        translated_matches = re.findall(format_regex, previous_string)
                        english_matches = re.findall(format_regex, input_node[j].text)
                        ans.append(match(translated_matches, english_matches, name_attr, out_lang, previous_string, input_node[j].text))
                        ans.append(contains_warning_char(name_attr, out_lang, previous_translated_text, input_node.text))
                        ans.append(check_xml_escaping(name_attr, out_lang, previous_translated_text, input_node.text))
                else:
                    raise ValueError
...
def main(argv):
    ...
    with Pool(args.pool) as p:
        ...
        arg_map = map(lambda it: ('en', it, args.i,
                                  args.o, debug), array_lang_striped)
